...
//         print_r($orderInfoTable->getOrderGoodsByOrderId(2844));
    ...
    public function receiveAction(){
        ...
        $usersTable = $this->getServiceLocator()->get('User\Model\UsersTable');
        ...
        foreach ($resultSet as $receiveOrder) {
            ...
            if((int)$result == 0){
                ...
                //save pay_points
                $user = $usersTable->fetch($receiveOrder->user_id);
                $user->pay_points = intval($user->pay_points) + intval($receiveOrder->money_paid);
...
//         exit;
    ...
    public function invalidAction(){
        set_time_limit(1800);
        date_default_timezone_set('Asia/Shanghai');
        $orderInfoTable = $this->getserviceLocator()->get('Timer\Model\UpdateLogisticTable');
        $resultSet = $orderInfoTable->fetchAll(array('status = 0','type = 1'));
        
        foreach ($resultSet as $ulRow) {
//             $ulRow = new UpdateLogistic();
